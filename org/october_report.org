...
These experiments bore out our hypothesis on return counts, in part. The mean count of returns per individual execution in the asexual, randomly-seeded /and/ the ROPgadget-seeded populations, over the course of 250 epochs, rarely exceeded 2 or 3.

For randomly-seeded populations equipped with single-point crossover, the mean return count was frequently double that, ranging between 4 and 7 across the three trials. The single-point crossover populations seeded with ROPgadget-harvested addresses showed mean return counts as high as 81, in one case, and between 12 and 15 in the other two. It's interesting to reflect that our asexual populations were unable to extract much benefit at all from these ROPgadget harvest initializations -- it seems likely that the high mutation rate in those populations had something to do with this. 
...
We were surprised by how weakly the populations equipped with alternating crossover performed. In most respects, they differed very little from the asexual populations: a maximum mean return count between 2 and 3, after 250 epochs, in the randomly-seeded populations, and between 4 and 5.5 in the ROPgadget-seeded populations.
